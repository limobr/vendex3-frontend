// src/screens/owner/EditProductScreen.js
...
const AttributeItem = ({ attribute, index, onUpdate, onRemove, onAddValue, error }) => {
    ...
    <View style={styles.attributeCard}>
      <View style={styles.attributeHeader}>
        <View style={styles.attributeHeaderLeft}>
          ...
          <TextInput
            style={[styles.attributeNameInput, error && styles.inputError]}
...
const VariantItem = ({ variant, index, onUpdate, onRemove, isBaseProduct = false, errors = {} }) => {
        ...
        <View style={styles.variantContent}>
          <View style={styles.variantRow}>
            <View style={styles.variantInputGroup}>
              ...
              <TextInput
                style={[styles.variantInput, errors.sku && styles.inputError]}
                value={variant.sku}
            ...
            <View style={styles.variantInputGroup}>
              ...
              <View style={styles.barcodeSection}>
                <View style={styles.barcodeInputContainer}>
                  <TextInput
                    style={[styles.variantInput, styles.barcodeInput, errors.barcode && styles.inputError]}
          ...
          <View style={styles.variantRow}>
            <View style={styles.variantInputGroup}>
              ...
              <View style={[styles.priceInputContainer, errors.cost_price && styles.inputError]}>
            ...
            <View style={styles.variantInputGroup}>
              ...
              <View style={[styles.priceInputContainer, (errors.selling_price || errors[`variant_${index}_selling_price`]) && styles.inputError]}>
                <Text style={styles.currencySymbol}>KES</Text>
                <TextInput
                  style={styles.priceInput}
              ...
              {(errors.selling_price || errors[`variant_${index}_selling_price`]) && (
                <Text style={styles.errorText}>{errors.selling_price || errors[`variant_${index}_selling_price`]}</Text>
              )}
            </View>
          </View>
          ...
          <View style={styles.variantRow}>
            <View style={styles.variantInputGroup}>
              ...
              <View style={[styles.priceInputContainer, errors.wholesale_price && styles.inputError]}>
                <Text style={styles.currencySymbol}>KES</Text>
                <TextInput
                  style={styles.priceInput}
              ...
              {errors.wholesale_price && (
                <Text style={styles.errorText}>{errors.wholesale_price}</Text>
              )}
            </View>
            {!isBaseProduct && (
              <View style={styles.variantInputGroup}>
                ...
                <TextInput
                  style={[styles.variantInput, errors.initial_stock && styles.inputError]}
...
const TaxSummary = ({ variants, baseSellingPrice, taxRate, taxInclusive, taxName, hasVariants }) => {
  ...
  const calculateTotalRevenue = () => {
    if (!hasVariants || variants.length === 0) {
      const price = parseFloat(baseSelling_price) || 0;
      const rate = parseFloat(taxRate) || 0;
...
export default function EditProductScreen({ route, navigation }) {
  const { productId, shopId, shopName, businessId } = route.params || {};
  const { user } = useAuth();
  ...
  }, [productId, businessId, user?.id]);

  // Load product data from API and database
  const loadProductData = async () => {
  try {
    setLoading(true);
    
    console.log("📦 Loading product data for ID:", productId);
    console.log("🔍 Checking if ID is UUID or nanoid...");
    
    // First check if this is a UUID or nanoid
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(productId);
    console.log(`📊 ID ${productId} is ${isUUID ? 'UUID' : 'nanoid'}`);
    
    if (isUUID) {
      // If it's a UUID, try to fetch from API
      try {
        console.log("🌐 Fetching from API with UUID...");
        const response = await productAPI.getProduct(productId);
        
        if (response.success && response.product) {
          const product = response.product;
          console.log("✅ Product data loaded from API:", product.name);
          setFormDataFromProduct(product);
        } else {
          throw new Error("API returned unsuccessful response");
        }
      } catch (apiError) {
        console.log("🌐 API fetch failed, trying local database...");
        // If API fails, try local database
        await loadProductFromLocalDatabase(productId);
      }
    } else {
      // If it's a nanoid, it's definitely offline data
      console.log("📱 Loading from local database (nanoid detected)...");
      await loadProductFromLocalDatabase(productId);
    }

    // Load categories and taxes
    ...
    // Try one more time with local database as fallback
    try {
      console.log("🔄 Trying fallback to local database...");
      await loadProductFromLocalDatabase(productId);
      await loadInitialData();
      setLoading(false);
    } catch (fallbackError) {
...
// New helper function to load product from local database
const loadProductFromLocalDatabase = async (id) => {
  try {
    console.log("📱 Searching in local database for ID:", id);
    
    // Try to get by local ID (nanoid)
    let localProduct = await databaseService.ProductService.getProductById(id, user?.id);
    
    // If not found by local ID, try to get by server_id (UUID)
    if (!localProduct) {
      console.log("🔄 Trying to find by server_id...");
      const products = await databaseService.ProductService.getProductsByBusiness(businessId, user?.id);
      localProduct = products.find(p => p.server_id === id);
    }
    ...
    console.log("✅ Found product in local database:", localProduct.name);
    
    // Get tax UUID if tax_id is a nanoid
    let taxServerId = localProduct.tax_server_id;
    if (!taxServerId && localProduct.tax_id) {
      const isTaxUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(localProduct.tax_id);
      if (!isTaxUUID) {
        // tax_id is a nanoid, need to get the server_id
        const localTax = await databaseService.TaxService.getTaxById(localProduct.tax_id, user?.id);
        if (localTax && localTax.server_id) {
          taxServerId = localTax.server_id;
          console.log("✅ Found tax server_id:", taxServerId);
        }
      } else {
        taxServerId = localProduct.tax_id;
      }
    }
    
    // Get category UUID if category_id is a nanoid
    let categoryServerId = localProduct.category_server_id;
    if (!categoryServerId && localProduct.category_id) {
      const isCategoryUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(localProduct.category_id);
      if (!isCategoryUUID) {
        // category_id is a nanoid, need to get the server_id
        const localCategory = await databaseService.CategoryService.getCategoryById(localProduct.category_id, user?.id);
        if (localCategory && localCategory.server_id) {
          categoryServerId = localCategory.server_id;
          console.log("✅ Found category server_id:", categoryServerId);
        }
      } else {
        categoryServerId = localProduct.category_id;
      }
    }
    
    // Update product with server_ids
    localProduct = {
      ...localProduct,
      tax_server_id: taxServerId,
      category_server_id: categoryServerId
    };
    
    // Get product with full details including variants and their attributes
    const productWithDetails = await databaseService.ProductService.getProductWithDetails(id, user?.id);
    
    if (productWithDetails) {
      console.log("📊 Product with details structure:", {
        variants: productWithDetails.variants?.length || 0,
        hasVariantsData: !!productWithDetails.variants,
        variantAttributes: productWithDetails.variants?.map(v => ({
          variantName: v.name,
          attributesCount: v.attributes?.length || 0,
          attributes: v.attributes
        }))
      });
      
      // Convert local product data to match API response format
      const transformedProduct = transformLocalProductForForm(productWithDetails, localProduct);
      setFormDataFromProduct(transformedProduct);
    } else {
      console.log("📊 No product details found, using basic product data");
      // If no details, try to get variants separately
      const variants = await databaseService.VariantService.getVariantsByProduct(id, user?.id);
      console.log("📊 Variants found separately:", variants?.length || 0);
      
      if (variants && variants.length > 0) {
        // Get attributes for each variant
        const variantsWithAttributes = await Promise.all(
          variants.map(async (variant) => {
            const variantAttributes = await databaseService.VariantAttributeService.getAttributesByVariant(
              variant.id, 
              user?.id
            );
            return {
              ...
              attribute_values: variantAttributes.map(attr => ({
        ...
        const transformedProduct = transformLocalProductForForm(productWithVariants, localProduct);
        setFormDataFromProduct(transformedProduct);
      } else {
        setFormDataFromProduct(localProduct);
      }
    }
...
// Helper function to transform local product data to match form structure
const transformLocalProductForForm = (productWithDetails, localProduct) => {
  console.log("🔄 Transforming local product data for form...");
  
  // Get variants from product details
  const variants = productWithDetails.variants || [];
  console.log("📊 Variants found:", variants.length);
  
  // Transform variants for form - use server_id when available
  const transformedVariants = variants.map((variant, index) => {
    // Parse attributes from variant name
    let attribute_values = [];
    
    // Try to parse attributes from variant name (format: "Size: M", "Color: Red / Size: L", etc.)
    if (variant.name && typeof variant.name === 'string') {
      console.log("📊 Parsing attributes from variant name:", variant.name);
      
      // Split by " / " for multiple attributes (e.g., "Color: Red / Size: M")
      const attributeParts = variant.name.split(' / ');
      
      attributeParts.forEach(part => {
        // Split by ": " to get attribute name and value
        const colonIndex = part.indexOf(': ');
        if (colonIndex !== -1) {
          const attribute_name = part.substring(0, colonIndex).trim();
    ...
    console.log(`📊 Parsed ${attribute_values.length} attributes from variant name:`, attribute_values);
    
    // CRITICAL: Use server_id for API calls, but store both
    const variantId = variant.server_id || variant.id;
    
    return {
      id: variantId, // Use server_id (UUID) for API calls
      local_id: variant.id, // Keep local id for database operations
      name: variant.name || `Variant ${index + 1}`,
      sku: variant.sku || "",
      barcode: variant.barcode || "",
      cost_price: variant.cost_price ? String(variant.cost_price) : "",
      selling_price: variant.selling_price ? String(variant.selling_price) : "",
      wholesale_price: variant.wholesale_price ? String(variant.wholesale_price) : "",
      initial_stock: variant.initial_stock ? String(variant.initial_stock) : "0",
  ...
  Object.keys(attributeMap).forEach(attrName => {
    attributes.push({
      name: attrName,
  ...
  // Get tax server_id if available
  let taxServerId = localProduct.tax_server_id || null;
  if (!taxServerId && localProduct.tax_id) {
    // Check if tax_id is already a UUID
    const isTaxUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(localProduct.tax_id);
    taxServerId = isTaxUUID ? localProduct.tax_id : null;
  }
  
  // Get category server_id if available
  let categoryServerId = localProduct.category_server_id || null;
  if (!categoryServerId && localProduct.category_id) {
    // Check if category_id is already a UUID
    const isCategoryUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(localProduct.category_id);
    categoryServerId = isCategoryUUID ? localProduct.category_id : null;
  }
  ...
  return {
    ...
    // Use server_id for API calls
    category_id: categoryServerId || localProduct.category_id,
    tax_id: taxServerId || localProduct.tax_id,
    business_id: localProduct.business_id,
    shop_id: localProduct.shop_id || shopId,
    has_variants: localProduct.has_variants || false,
    variant_type: localProduct.variant_type || "none",
    base_barcode: localProduct.base_barcode || "",
    base_sku: localProduct.base_sku || "",
    base_cost_price: localProduct.base_cost_price ? String(localProduct.base_cost_price) : "",
    base_selling_price: localProduct.base_selling_price ? String(localProduct.base_selling_price) : "",
    base_wholesale_price: localProduct.base_wholesale_price ? String(localProduct.base_wholesale_price) : "",
    tax_inclusive: localProduct.tax_inclusive !== false,
    is_trackable: localProduct.is_trackable !== false,
    is_active: localProduct.is_active !== false,
    // Add server_id if available
    server_id: localProduct.server_id || null
  };
};


// Updated setFormDataFromProduct function
const setFormDataFromProduct = (product) => {
  console.log("📝 Setting form data from product:", product.name);
  console.log("🔑 Setting server ID:", product.server_id);
  
  // Set the server UUID
  setServerProductId(product.server_id);
  
  // Parse attributes from existing variants if available
  ...
  if (product.variants && product.variants.length > 0) {
    ...
    product.variants.forEach(variant => {
      // Try to get attributes from variant.attribute_values first
      if (variant.attribute_values && variant.attribute_values.length > 0) {
        variant.attribute_values.forEach(attr => {
          const attrName = attr.attribute_name;
          const attrValue = attr.value;
      ...
      // If no attribute_values, try to parse from variant name
      else if (variant.name) {
        const parsedAttributes = parseVariantAttributes(variant.name);
    ...
    // Convert the attribute map to the format needed for the form
    extractedAttributes = Object.keys(attributeMap).map(attrName => ({
      name: attrName,
      values: Array.from(attributeMap[attrName])
    }));
  }
  ...
  setForm({
    ...
    base_cost_price: product.base_cost_price ? String(product.base_cost_price) : "",
    base_selling_price: product.base_selling_price ? String(product.base_selling_price) : "",
    base_wholesale_price: product.base_wholesale_price ? String(product.base_wholesale_price) : "",
    tax: product.tax_id || null,
    tax_inclusive: product.tax_inclusive !== false,
    unit_of_measure: product.unit_of_measure || "pcs",
    reorder_level: product.reorder_level ? String(product.reorder_level) : "10",
    is_trackable: product.is_trackable !== false,
    is_active: product.is_active !== false,
    business: product.business_id || businessId,
    shop: product.shop_id || shopId,
  });

  // Set variant states
  setHasVariants(product.has_variants || false);
  setVariantType(product.variant_type || "none");

  // Set attributes - use extracted attributes if they exist, otherwise use product.attributes
  ...
  if (product.variants && product.variants.length > 0) {
    const transformedVariants = product.variants.map(variant => {
      // Parse attribute values if not already present
      let attribute_values = [];
      if (variant.attribute_values && variant.attribute_values.length > 0) {
        attribute_values = variant.attribute_values;
      } else if (variant.name) {
        attribute_values = parseVariantAttributes(variant.name);
      }
      
      // Use server_id (UUID) for the id field when sending to API
      // The local_id is kept for tracking but not sent to API
      return {
        id: variant.id, // This should be server_id (UUID) if available
        local_id: variant.local_id || variant.id, // Store original local id
        server_id: variant.server_id, // Keep server_id reference
        name: variant.name || "",
        sku: variant.sku || "",
        barcode: variant.barcode || "",
        cost_price: variant.cost_price ? String(variant.cost_price) : "",
    ...
    console.log("📊 Transformed variants for form:", transformedVariants.map(v => ({
      id: v.id,
  ...
  const fetchUpdatesInBackground = async () => {
    try {
      if (businessId) {
        const categoriesResponse = await productAPI.getCategories(businessId);
  ...
  const newVariants = combinations.map((combo, index) => {
    // Create variant name in format "Attribute1: Value1 / Attribute2: Value2"
...
const parseVariantAttributes = (variantName) => {
  ...
  if (variantName && typeof variantName === 'string') {
    ...
    const attributeParts = variantName.split(' / ');
...
const formatAttributesForSubmission = (attributes) => {
...
// Update the handleSubmit function to use this helper
const convertToServerUUID = async (localId, serviceMethod, userId) => {
  ...
  // It's a nanoid, try to get server_id from local database
  try {
    const localItem = await serviceMethod(localId, userId);
    if (localItem && localItem.server_id) {
      return localItem.server_id;
    }
  } catch (error) {
    console.log(`⚠️ Could not find server_id for ${localId}:`, error);
  }
...
const handleSubmit = async () => {
  ...
  try {
    // First, get the current product from local database to check its data
    let localProduct;
    try {
      localProduct = await databaseService.ProductService.getProductById(productId, user?.id);
      console.log("📱 Found local product:", localProduct?.name);
    } catch (error) {
      console.log("📱 Could not find local product:", error);
    }
    
    // Get the server UUID for the product
    const productServerId = localProduct?.server_id || serverProductId || productId;
    console.log(`🔑 Using product server ID: ${productServerId} (local: ${productId})`);
    
    // Convert tax_id to server UUID
    let taxServerId = await convertToServerUUID(
      form.tax,
      databaseService.TaxService.getTaxById,
      user?.id
    );
    console.log(`💰 Tax ID conversion: ${form.tax} → ${taxServerId}`);
    
    // Convert category_id to server UUID
    let categoryServerId = await convertToServerUUID(
      form.category,
      databaseService.CategoryService.getCategoryById,
      user?.id
    );
    console.log(`📂 Category ID conversion: ${form.category} → ${categoryServerId}`);
    
    const productData = {
      name: form.name.trim(),
      description: form.description.trim(),
      category_id: categoryServerId || null,
      ...
      attributes: hasVariants && variantType !== "none" ? formatAttributesForSubmission(attributes) : [],
      variants: hasVariants && variantType !== "none" ? variants.map(variant => {
        // For variants, we need to check if the ID is a UUID
        const variantId = variant.id && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(variant.id) 
          ? variant.id 
          : undefined;
        
        return {
          id: variantId, // Only include if it's a UUID
    ...
    console.log("🔄 Updating product with data:", {
      ...
      variantIds: productData.variants?.map(v => ({ id: v.id, name: v.name })),
      tax_id: productData.tax_id,
      category_id: productData.category_id
    });
    
    const response = await productAPI.updateProduct(productServerId, productData);
    
    if (response.success) {
      // Update the local database with the server response
      if (response.product && localProduct) {
        // Update the main product
        await databaseService.ProductService.saveProduct({
          ...localProduct,
          ...response.product,
          server_id: response.product.id || localProduct.server_id,
          tax_server_id: response.product.tax_id || null,
          category_server_id: response.product.category_id || null,
          last_synced: new Date().toISOString(),
          sync_status: 'synced',
        }, user?.id);
        
        // If variants were updated, update them in local database
        if (response.product.variants && Array.isArray(response.product.variants)) {
          for (const serverVariant of response.product.variants) {
            // Find the matching local variant by SKU or name
            const matchingLocalVariant = variants.find(v => 
              v.sku === serverVariant.sku || 
              v.name === serverVariant.name ||
              (v.local_id && v.local_id === serverVariant.local_id)
            );
            
            if (matchingLocalVariant) {
              await databaseService.VariantService.saveVariant({
                id: matchingLocalVariant.local_id || matchingLocalVariant.id,
                server_id: serverVariant.id,
                product_id: localProduct.id,
                name: serverVariant.name,
                sku: serverVariant.sku,
                barcode: serverVariant.barcode,
                cost_price: serverVariant.cost_price,
                selling_price: serverVariant.selling_price,
                wholesale_price: serverVariant.wholesale_price,
                is_default: serverVariant.is_default,
                is_active: serverVariant.is_active !== false,
                last_synced: new Date().toISOString(),
                sync_status: 'synced',
              }, user?.id);
            }
                ...
                {variantType !== "none" && (
                  <>
                    ...
                    <View style={styles.attributesSection}>
                      ...
                      {errors.variants && (
                        <Text style={[styles.errorText, styles.variantError]}>
                          {errors.variants}
                        </Text>
                      )}
                    </View>
          ...
          {saving ? (
            ...
            <>
              ...
              <Text style={styles.submitButtonText}>
                {hasVariants ? `Update Product (${variants.length} variants)` : "Update Product"}
              </Text>
            </>
          )}
        </TouchableOpacity>
